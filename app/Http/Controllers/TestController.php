...
class TestController extends Controller
{
    ...
    //product import from cutprice bd
    public function importProduct2(){
        ...
        foreach($array as $data) {
            $product = Product::where('title',$data['name'])->first();
            if (!isset($product->id)){
                $temp = [
                    'status' => $data['status'],
                    'title' => $data['name'],
                    ...
                    'meta_tags' => $data['tags'],
                    'image' => 'digital_ocean',
                    'image_path' => json_encode($data['img_paths']),
                    'media_id' => null,
                    'stock' => $data['stock'],
                    'sale_price' => $data['last_price'],
                    'regular_price' => $data['first_price']
                ];

                if (isset($data['Brand']) && $data['Brand']) {
                    $brand = Brand::where('title', $data['Brand']['name'])->first();
                    if (!isset($brand->id)) {
                        $brand = new Brand;
                        $brand->title = $data['Brand']['name'];
                        $brand->slug = Str::slug($data['Brand']['name']);
                        $brand->slug = Str::slug($data['Brand']['name']);
                        $brand->save();
                    }
                    $temp +=[
                ...
                $img = collect($data['img_paths']);
                if (isset($img['original']) && $img['original']) {
                    $uploaded_file = MediaRepo::uploadUrl($img['original']);
                    if ($uploaded_file['status']) {
                        $temp +=[
                ...
                $product = Product::create($temp);

                $product_data = new ProductData();
                $product_data->type = 'Simple';
                $product_data->product_id = $product->id;
                $product_data->regular_price = $data['first_price'];
                $product_data->sale_price = $data['last_price'];

                $product_data->sku_code = $data['general_meta']['SKU'] ?? null;
                $product_data->shipping_weight = $data['general_meta']['weight'];
                $product_data->shipping_width = null;
                $product_data->shipping_height = null;
                $product_data->unit = $data['general_meta']['unit'] ?? null;
                $product_data->unit_amount = $data['general_meta']['selling_price'];
                $product_data->stock = $data['general_meta']['stock'];
                $product_data->save();

                if (isset($data['categories_rel']) && count($data['categories_rel'])){
                    foreach ($data['categories_rel'] as $cat) {
                        // Create product category
                        $category = Category::where('title', $cat['name'])->first();
                        ...
                        if (!isset($category->id)) {
                            ...
                            $category->feature = $cat['featured'];
                            $category->home_block = $cat['block'];
                            $category->category_id = $cat['parent'];
                            $category->status = $cat['status'];
                            $category->save();
                        }
                        ProductRepo::insertProductCategory($product->id, $category->id);

                        if (isset($cat['sub_category']) && count($cat['sub_category'])) {
                            foreach ($data['categories_rel'] as $cat2) {
                                $sub_category = Category::where('title', $cat2['name'])->first();

                                if (!isset($sub_category->id)) {
                                    $sub_category = new Category;
                                    $sub_category->title = $cat['name'];
                                    $sub_category->for = 'product';
                                    $sub_category->slug = Str::slug($cat['name']);
                                    $sub_category->feature = $cat['featured'] ?? 0;
                                    $sub_category->home_block = $cat['block'] ?? 0;
                                    $sub_category->category_id = $cat['parent'] ?? null;
    ...
    {
        $dd = $this->sentSmsCustom('test sms from local', '01784222255', '88');
        dd($dd);
    }*/
}
