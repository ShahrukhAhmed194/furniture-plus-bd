...
class FrontendController extends Controller
{
    ...
    public function generalStore(Request $request){
        ...
        $where['name'] = 'whatsapp_number';
        $insert['value'] = $request->whatsapp_number;
        DB::table('settings')->updateOrInsert($where, $insert);
        ...
        $where['name'] = 'appstore';
        $insert['value'] = $request->appstore;
        DB::table('settings')->updateOrInsert($where, $insert);

        $where['name'] = 'playstore';
        $insert['value'] = $request->playstore;
        DB::table('settings')->updateOrInsert($where, $insert);
        ...
        $where['name'] = 'stock_manage';
        $insert['value'] = $request->stock_manage;
        DB::table('settings')->updateOrInsert($where, $insert);

        $where['name'] = 'template';
        $insert['value'] = $request->template;
        DB::table('settings')->updateOrInsert($where, $insert);
        ...
        if($request->footer_logo){
            $this->validate($request, [
                'footer_logo' => 'image|mimes:jpg,png,jpeg,gif,web'
            ]);
        ...
        if($request->favicon){
            $this->validate($request, [
                'favicon' => 'image|mimes:jpg,png,jpeg,gif,web'
            ]);
        ...
        if($request->og_image){
            $this->validate($request, [
                'og_image' => 'image|mimes:jpg,png,jpeg,gif,web'
            ]);
        ...
        if($request->homepage_banner_image){
            $this->validate($request, [
                'homepage_banner_image' => 'image|mimes:jpg,png,jpeg,gif,web'
            ]);
    ...
    {
        $query = App::where('group', 'smsTemplate')->get();
        return view('back.frontend.sms-settings')->with([
            'query' => $query
        ]);
    }

    public function smsInfoUpdate(Request $request)
    {
        $request->validate([
            'smsProvider'      => 'required|max:191|string',
            'username'      => 'required|max:191|string',
            'password'   => 'required|string|max:191'
        ]);
        $where = array();
        if ($request->group == 'sms') {
            $where['group'] = 'sms';
            if ($request->input('smsProvider')) {
                $where['name'] = 'smsProvider';
                $insert['value'] = $request->smsProvider;
            ...
            if ($request->input('username')) {
                $where['name'] = 'username';
                $insert['value'] = $request->username;
                App::updateOrInsert($where, $insert);
            }
            if ($request->input('password')) {
                $where['name'] = 'password';
                $insert['value'] = $request->password;
                App::updateOrInsert($where, $insert);
            }
        }
        return redirect()->back()->with('success-alert', 'Information updated successful.');
    ...
    public function addSMSTemplate(Request $request){
        ...
        if ($request->id){
            $query = App::find($request->id);
        }else{
            $query = new App();
        }

        $query->group = 'smsTemplate';
        $query->name = $request->name;
        $query->value = $request->massage;
    ...
    public function changeApp(Request $request){
        $query = App::where('group', $request->group)->where('name', $request->name)->first();
