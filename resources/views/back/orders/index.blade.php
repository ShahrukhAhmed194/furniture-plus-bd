...
@section('master')
    <div class="card border-light mt-3 shadow">
        <div class="card-header">
            ...
            <a href="{{route('back.orders.create')}}" class="btn btn-warning btn-sm float-right "><i class="fas fa-plus"></i> Create</a>
        </div>
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <button type="button" class="btn btn-sm btn-warning mt-4 print_selected_btn2"><i class="fas fa-print"></i> Print Selected 2</button>
                    </div>
        ...
        <form id="print_form_2" action="{{route('back.orders.printList2')}}" method="POST">
            @csrf
            <div id="form_data_2"></div>
        </form>
    </div>
@endsection
    ...
    <script>
        ...
        $(document).on('click', '.print_selected_btn2', function(){
            ...
            if(!orders.get().length){
                cAlert('error', 'Please select some order!');
            }else{
                let html = ''
                let ods = orders.get()
                for(let i=0; i < orders.get().length; i++) html+="<input type='hidden' name='orders[]' value='"+ods[i]+"'>"
                $('#form_data_2').html(html)
                document.getElementById('print_form_2').submit()
            }
        });
    </script>
