<div class="col-span-5 flex">
    {{--<button data-collapse-toggle="mega-menu-full" type="button" id="page_loader" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200" aria-controls="mega-menu-full" aria-expanded="false">
    ...
    <div class="ml-4 lg:ml-0">
        <a href="{{route('homepage')}}">
            <img class="max-w-full object-contain object-left w-full h-[2.5rem] md:h-[4rem]" src="{{$settings_g['logo'] ?? ''}}" alt="{{$settings_g['title'] ?? env('APP_NAME')}}">
        </a>
    </div>
</div>
<div class="col-span-7">
    <div class="flex justify-end gap-2">
        <form action="{{route('search')}}" class="border-[#22222226] border rounded-full justify-center overflow-hidden w-full max-w-full h-[2.5rem] hidden md:flex" method="get">
            <input type="text" name="search" value="{{request('search')}}" class="w-full focus:outline-none p-2 ml-2 border-none placeholder:font-[300] font-[300]" placeholder="Search for products">
